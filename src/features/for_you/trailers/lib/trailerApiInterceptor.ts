...
class TrailerApiInterceptor {
  ...
  constructor() {
    this.baseURL = TRAILER_API_CONFIG.baseURL;
  ...
  async request<T>(
    ...
    const {
      isPublic = false,
      cacheKey,
      cacheTTL = TRAILER_CACHE_TTL.trailers,
    ...
    try {
      ...
      if (method === TRAILER_HTTP_METHODS.GET && cacheKey && response.success) {
  ...
  private async retryRequest<T>(
    ...
    retries: number = TRAILER_API_CONFIG.retryAttempts,
    delay: number = TRAILER_API_CONFIG.retryDelay
  ): Promise<TrailerApiResponse<T>> {
    try {
