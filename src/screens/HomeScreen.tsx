          ...
          {isSelected === 'all' ? (
            <View style={style.container}>
              ...
              <TopContentSection
                title="Top 10 Shows"
                data={(() => {
                  const data = topContentData?.data?.top || [];
                  
                  const processedData = data.map((item: any) => ({
                    ...item,
                    backdropImage: item.backdropImage || item.contentDetails?.backdropImage,
                    image: item.image || item.contentDetails?.posterImage,
                    posterImage: item.posterImage || item.contentDetails?.posterImage,
                    thumb: item.thumb || item.contentDetails?.thumb,
                  }));
                  
                  return processedData;
                })()}
                onSeeAll={() => {}}
                renderItem={props => (renderTopMovieCard({ ...props, navigation, style }) as React.ReactElement) || <View key="fallback" />}
                loadingStates={loadingStates}
                style={style}
                isLargeDevice={isLargeDevice}
                appFonts={appFonts}
